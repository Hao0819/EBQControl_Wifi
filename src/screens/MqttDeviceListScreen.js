...
// import { View, StyleSheet, FlatList, TouchableOpacity, Text, Alert, Switch } from 'react-native';
...
//     title: 'EBQ Control',
//     gestureEnabled: false,       // iOS 禁止侧滑返回（可选）
//     headerLeft: () => null,      // ✅ 关键：隐藏左上角返回箭头

//     headerRight: () => (
//       <View style={{ flexDirection: 'row', alignItems: 'center' }}>
//         <Text style={{ marginRight: 6, color: '#1E293B' }}>Wi-Fi</Text>
//         <Switch
//           value={true}
//           onValueChange={() => {
//             navigation.reset({
...
//             );
//           }}
//           style={{ paddingHorizontal: 14, marginLeft: 10 }}
//         >
...
// ⚠️ Must match the route name in App.tsx / App.js
// e.g. <Stack.Screen name="Scanner" component={BleScanner} />
const BLE_ROUTE = 'Scanner';
export default function MqttDeviceListScreen({ navigation }) {
    const [devices, setDevices] = useState([]);
    const [mqttState, setMqttState] = useState(
        MqttManager.getMqttState?.() ?? {
            deviceId: null, connected: false,
            connecting: false, status: 'IDLE', error: '',
        }
    );

    // ── Subscribe to global MQTT state ────────────────────────────────────────
    useEffect(() => {
        return MqttManager.subscribeMqttState?.((next) => {
            setMqttState((prev) => {
                const same =
    ...
    // ── Switch to BLE scanner ─────────────────────────────────────────────────
    const switchToBle = useCallback(() => {
        navigation.reset({
            index: 0,
            routes: [{ name: BLE_ROUTE }],
        });
    }, [navigation]);
        ...
        navigation.setOptions({
            ...
            headerRight: () => (
                <View style={styles.headerRight}>
                    {/* BLE / WiFi toggle — value=true because this IS the WiFi screen */}
                    <Text style={styles.switchLabel}>Wi-Fi</Text>
                    <Switch
                        value={true}
                        onValueChange={switchToBle}
                        trackColor={{ false: '#2196F3', true: '#10B981' }}
                        thumbColor="#FFFFFF"
                        style={styles.switch}
                    />
    ...
    useEffect(() => { console.log('[MQTT-LIST] mounted'); }, []);
    ...
    return (
        <View style={styles.page}>
            ...
            {/* Floating Add button */}
            <TouchableOpacity style={styles.fab} onPress={() => navigation.navigate('AddMqttDevice')} activeOpacity={0.9}>
                <Text style={styles.fabText}>+</Text>
            </TouchableOpacity>
        </View>
    );
...
const styles = StyleSheet.create({
    ...
    switch: {
        transform: Platform.OS === 'android' ? [{ scaleX: 0.85 }, { scaleY: 0.85 }] : [],
    },
    headerButton: {
        paddingHorizontal: 4,
        marginLeft: 4,
